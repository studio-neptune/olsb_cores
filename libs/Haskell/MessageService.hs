...
from_FetchMessageOperations_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v3040 -> P.Just (2, ("localRevision",T.TI64 _v3040))) $ fetchMessageOperations_args_localRevision record
  , (\_v3040 -> P.Just (3, ("lastOpTimestamp",T.TI64 _v3040))) $ fetchMessageOperations_args_lastOpTimestamp record
  , (\_v3040 -> P.Just (4, ("count",T.TI32 _v3040))) $ fetchMessageOperations_args_count record
  ]
...
to_FetchMessageOperations_args (T.TStruct fields) = FetchMessageOperations_args{
  fetchMessageOperations_args_localRevision = P.maybe (fetchMessageOperations_args_localRevision default_FetchMessageOperations_args) (\(_,_val3042) -> (case _val3042 of {T.TI64 _val3043 -> _val3043; _ -> P.error "wrong type"})) (Map.lookup (2) fields),
  fetchMessageOperations_args_lastOpTimestamp = P.maybe (fetchMessageOperations_args_lastOpTimestamp default_FetchMessageOperations_args) (\(_,_val3042) -> (case _val3042 of {T.TI64 _val3044 -> _val3044; _ -> P.error "wrong type"})) (Map.lookup (3) fields),
  fetchMessageOperations_args_count = P.maybe (fetchMessageOperations_args_count default_FetchMessageOperations_args) (\(_,_val3042) -> (case _val3042 of {T.TI32 _val3045 -> _val3045; _ -> P.error "wrong type"})) (Map.lookup (4) fields)
  }
...
from_FetchMessageOperations_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v3048 -> (1, ("e",from_TalkException _v3048))) <$> fetchMessageOperations_result_e record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v3048 -> P.Just (0, ("success",from_MessageOperations _v3048))) $ fetchMessageOperations_result_success record
    , (\_v3048 -> (1, ("e",from_TalkException _v3048))) <$> fetchMessageOperations_result_e record
    ]
    )
...
to_FetchMessageOperations_result (T.TStruct fields) = FetchMessageOperations_result{
  fetchMessageOperations_result_success = P.maybe (fetchMessageOperations_result_success default_FetchMessageOperations_result) (\(_,_val3050) -> (case _val3050 of {T.TStruct _val3051 -> (to_MessageOperations (T.TStruct _val3051)); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  fetchMessageOperations_result_e = P.maybe (P.Nothing) (\(_,_val3050) -> P.Just (case _val3050 of {T.TStruct _val3052 -> (to_TalkException (T.TStruct _val3052)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
...
from_GetLastReadMessageIds_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v3055 -> P.Just (2, ("chatId",T.TString $ E.encodeUtf8 _v3055))) $ getLastReadMessageIds_args_chatId record
  ]
...
to_GetLastReadMessageIds_args (T.TStruct fields) = GetLastReadMessageIds_args{
  getLastReadMessageIds_args_chatId = P.maybe (getLastReadMessageIds_args_chatId default_GetLastReadMessageIds_args) (\(_,_val3057) -> (case _val3057 of {T.TString _val3058 -> E.decodeUtf8 _val3058; _ -> P.error "wrong type"})) (Map.lookup (2) fields)
  }
...
from_GetLastReadMessageIds_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v3061 -> (1, ("e",from_TalkException _v3061))) <$> getLastReadMessageIds_result_e record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v3061 -> P.Just (0, ("success",from_LastReadMessageIds _v3061))) $ getLastReadMessageIds_result_success record
    , (\_v3061 -> (1, ("e",from_TalkException _v3061))) <$> getLastReadMessageIds_result_e record
    ]
    )
...
to_GetLastReadMessageIds_result (T.TStruct fields) = GetLastReadMessageIds_result{
  getLastReadMessageIds_result_success = P.maybe (getLastReadMessageIds_result_success default_GetLastReadMessageIds_result) (\(_,_val3063) -> (case _val3063 of {T.TStruct _val3064 -> (to_LastReadMessageIds (T.TStruct _val3064)); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  getLastReadMessageIds_result_e = P.maybe (P.Nothing) (\(_,_val3063) -> P.Just (case _val3063 of {T.TStruct _val3065 -> (to_TalkException (T.TStruct _val3065)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
...
from_MultiGetLastReadMessageIds_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v3068 -> P.Just (2, ("chatIds",T.TList T.T_STRING $ P.map (\_v3070 -> T.TString $ E.encodeUtf8 _v3070) $ Vector.toList _v3068))) $ multiGetLastReadMessageIds_args_chatIds record
  ]
...
to_MultiGetLastReadMessageIds_args (T.TStruct fields) = MultiGetLastReadMessageIds_args{
  multiGetLastReadMessageIds_args_chatIds = P.maybe (multiGetLastReadMessageIds_args_chatIds default_MultiGetLastReadMessageIds_args) (\(_,_val3072) -> (case _val3072 of {T.TList _ _val3073 -> (Vector.fromList $ P.map (\_v3074 -> (case _v3074 of {T.TString _val3075 -> E.decodeUtf8 _val3075; _ -> P.error "wrong type"})) _val3073); _ -> P.error "wrong type"})) (Map.lookup (2) fields)
  }
...
from_MultiGetLastReadMessageIds_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v3078 -> (1, ("e",from_TalkException _v3078))) <$> multiGetLastReadMessageIds_result_e record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v3078 -> P.Just (0, ("success",T.TList (T.T_STRUCT typemap_LastReadMessageIds) $ P.map (\_v3080 -> from_LastReadMessageIds _v3080) $ Vector.toList _v3078))) $ multiGetLastReadMessageIds_result_success record
    , (\_v3078 -> (1, ("e",from_TalkException _v3078))) <$> multiGetLastReadMessageIds_result_e record
    ]
    )
...
to_MultiGetLastReadMessageIds_result (T.TStruct fields) = MultiGetLastReadMessageIds_result{
  multiGetLastReadMessageIds_result_success = P.maybe (multiGetLastReadMessageIds_result_success default_MultiGetLastReadMessageIds_result) (\(_,_val3082) -> (case _val3082 of {T.TList _ _val3083 -> (Vector.fromList $ P.map (\_v3084 -> (case _v3084 of {T.TStruct _val3085 -> (to_LastReadMessageIds (T.TStruct _val3085)); _ -> P.error "wrong type"})) _val3083); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  multiGetLastReadMessageIds_result_e = P.maybe (P.Nothing) (\(_,_val3082) -> P.Just (case _val3082 of {T.TStruct _val3086 -> (to_TalkException (T.TStruct _val3086)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
