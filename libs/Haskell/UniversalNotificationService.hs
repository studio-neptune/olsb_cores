...
from_Notify_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v6374 -> P.Just (2, ("event",from_GlobalEvent _v6374))) $ notify_args_event record
  ]
...
to_Notify_args (T.TStruct fields) = Notify_args{
  notify_args_event = P.maybe (notify_args_event default_Notify_args) (\(_,_val6376) -> (case _val6376 of {T.TStruct _val6377 -> (to_GlobalEvent (T.TStruct _val6377)); _ -> P.error "wrong type"})) (Map.lookup (2) fields)
  }
...
from_Notify_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v6380 -> (1, ("e",from_UniversalNotificationServiceException _v6380))) <$> notify_result_e record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v6380 -> (1, ("e",from_UniversalNotificationServiceException _v6380))) <$> notify_result_e record
    ]
    )
...
to_Notify_result (T.TStruct fields) = Notify_result{
  notify_result_e = P.maybe (P.Nothing) (\(_,_val6382) -> P.Just (case _val6382 of {T.TStruct _val6383 -> (to_UniversalNotificationServiceException (T.TStruct _val6383)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
