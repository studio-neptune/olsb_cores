    ...
    elseif fid == 2 then
      if ftype == TType.LIST then
        ...
        for _i=1,_size574 do
          ...
          table.insert(self.chatIds, _elem578)
        end
        iprot:readListEnd()
      else
        iprot:skip(ftype)
...
function multiGetLastReadMessageIds_args:write(oprot)
  ...
  if self.chatIds ~= nil then
    ...
    for _,iter579 in ipairs(self.chatIds) do
    ...
    elseif fid == 0 then
      if ftype == TType.LIST then
        ...
        for _i=1,_size580 do
          local _elem584 = LastReadMessageIds:new{}
          _elem584:read(iprot)
          table.insert(self.success, _elem584)
        end
        iprot:readListEnd()
      else
        iprot:skip(ftype)
...
function multiGetLastReadMessageIds_result:write(oprot)
  ...
  if self.success ~= nil then
    ...
    for _,iter585 in ipairs(self.success) do
      iter585:write(oprot)
    end
    oprot:writeListEnd()
    oprot:writeFieldEnd()
